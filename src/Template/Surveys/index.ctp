<div id="container" class="ov">
    <div id="content-left">
        ...
        ?>
        <div id="filterLeft">
            <a href="<?php echo $this->Url->build(['controller' => 'Surveys', 'action' => 'index?filter=user&type=' . $typeUser]); ?>">
            ...
            <a href="<?php echo $this->Url->build(['controller' => 'Surveys', 'action' => 'index?filter=id&type=' . $typeId]); ?>">
                <div class="filterId">
                    <span>
                        <?= __('SURVEY ID') ?>
                    </span>
            ...
            <a href="<?php echo $this->Url->build(['controller' => 'Surveys', 'action' => 'index?filter=date&type=' . $typeDate]); ?>">
        ...
        <div id="list_records">
            ...
            <?php if (!isset($id)) $id = $arrayIndexReverse[$routeArrays[0]['id']] ?>
<?php foreach ($routeArrays as $key => $route) : ?>
                <div class="item ov mb10" id="<?= $arrayIndexReverse[$route['id']] ?>">
    ...
    <?php endif; ?>
                    ...
                    <div class="col-lg-4 showData">
                        ...
                        ?>
                        <a rel="<?= $url ?>" class="target targetOnly">
...
//            console.log(datum);
    ...
    function getCoordinates(id) {
        $.ajax({
            url: '<?= $this->Url->build(["controller" => "surveys", "action" => "getDataAjax"]); ?>',
            dataType: "json",
            data: {id: id, first: 1},
            type: "post",
            async: false,
...
<?php endif; ?>
                    $(".target").click(function (e) {
                        e.preventDefault();
                        console.log('aaâxxx');
//                        var id = $(this).prev().attr('data-bind');
